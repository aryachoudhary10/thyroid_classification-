...
ConvNeXt-Tiny backbone. Reliability diagrams are computed directly from raw
predicted probabilities; ECE bars come from calibration_table.csv, which the
pipeline produces via temperature scaling fit on development-set OOF only
(never on the test cohort).
"""
...
def reliability_bins(y: np.ndarray, p: np.ndarray, n_bins: int = 10):
    ...
    for b in range(n_bins):
        ...
        acc.append(y[m].mean())
    return np.array(mean_p), np.array(acc)
...
def main() -> None:
    ...
    for m in MODELS:
        d = data.load_predictions(data.CONVNEXT_PRED[m], pcol="p_raw")
        mp, acc = reliability_bins(d.label.to_numpy(), d.p.to_numpy())
        ax.plot(mp, acc, marker="o", markersize=5, linewidth=1.8,
               color=MODEL_COLORS[m], label=MODEL_LABELS[m].split(" (")[0])
    style_ax(ax, "(a) Reliability diagram -- raw probabilities\nConvNeXt-Tiny, ThyroidXL test, n=739, 10 bins",
    ...
    rows = []
    for m in MODELS:
        cal = pd.read_csv(data.require(data.CONVNEXT_CAL[m]))
    ...
    ax.set_xticks(x); ax.set_xticklabels([MODEL_LABELS[r[0]].split(" (")[0] for r in rows],
                                          fontsize=9)
    style_ax(ax, "(b) Expected calibration error, ConvNeXt-Tiny\nThyroidXL test",
    ...
    caption = (
        ...
        "cohort, ConvNeXt-Tiny backbone. (a) Reliability diagrams built "
        "directly from each model's raw predicted probabilities (10 "
        "equal-width bins). (b) Expected calibration error (ECE) comparing "
        ...
        "malignancy prevalence between development (26.1%%) and test "
        "(47.8%%). The calibration temperature and correction are both fit on "
        "out-of-fold development predictions only and applied unchanged to "
        "the test cohort."
    )
    style.save(fig, "fig05_calibration_convnext", caption)
